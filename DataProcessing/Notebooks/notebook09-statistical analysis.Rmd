...
```{r}
rm(list=ls(all=TRUE))
load("~/Post-doc/Data/Total Merged Data File (March 6 2024).RData")
###str(CC.TotalData)

AGG_Dark <- aggregate(FeedingDark, by = list(FeedingDark$D_V_T, FeedingDark$Chem, FeedingDark$Flow.rate), FUN = mean)
head(AGG_Dark)
AGG_Dark <- AGG_Dark[ -c(4:5, 9:18, 54:56)]
colnames(AGG_Dark) <- c("D_V_T", "Chem", "Flow.rate", "X", "Y", "Z", "dx", "dy", "dz", "d", "vx", "vy", "vz", "v", "h.heading", "v.heading", "turn.anglexy", "turn.angleyz", "turn.angle", "vel.turn.angle", "vel", "vel.flow", "trim.X", "xsmooth", "ysmooth", "zsmooth", "smooth.dx", "smooth.dy", "smooth.dz", "smooth.d", "smooth.vx", "smooth.vy", "smooth.vz", "smooth.v", "smooth.h.heading", "heading.pi", "smooth.v.heading", "pitch.perfect", "turn.anglexysmooth", "turn.angleyzsmooth", "turn.angle.smooth")
head(AGG_Dark)
tail(AGG_Dark)


AGG_Dark$Flow.Rate <- as.factor(AGG_Dark$Flow.Rate)
AGG_Dark$Chemical.Cue <- as.factor(AGG_Dark$Chemical.Cue)
```
...
Feeding <- rbind(LM, LH, DM, DH)
str(AGG_Feed)
Feeding$Chem <- as.factor(Feeding$Chem)
levels(FeedingDark$Chem)
...
contrasts(Feeding$Chem)=contr.poly(3)
##then run your model as

head(AGG_Dark)

model.1=aov(dv~covariate+factorvariable, data=dataname) ## example
mod1=aov(smooth.vel ~ Flow.Rate*Chemical.Cue, data=AGG_Dark)
mod2 <- update(mod1, ~.-Chemical.Cue:Flow.Rate)
...
summary.lm(mod1) ##will give another summary and includes the R-sq. output. Example
summary.lm(mod2)

posth=glht(model.1, linfct=mcp(factorvariable="Tukey"))  ##gives the post-hoc Turkey analysis. Example
posth=glht(mod1, linfct=mcp(Chemical.Cue*Flow.Rate ="Tukey"))
summary(posth) ##shows the output in a nice format.

TukeyHSD(mod1, conf.level=.95) ## gives full Tukey post-hoc for all pairwise combinations
...
mod2=aov(vel.flow~Flow.rate+Chem+Flow.rate:Chem, data=AGG_Feed) ## not right?
##If the interaction term is significant then you do not have homogeneity.

##################################

##linear mixed effect models
fit <- aov(v ~ Chem*Light*Flow.rate, data=AGG_Feed)
fit2 <- update(fit, ~.-Chem:Flow.rate:Light)# same thing minus 3-way interaction

##par(mfrow=c(2,2), mar=c(2,2,2,2), oma=c(2,2,2,2)) # optional layout 
##plot(fit) # diagnostic plots

summary(fit2) # display Type I ANOVA table
drop1(mod1,~.,test="F") # type III SS and F Tests
...
## boxplots for velocity for each level of flow
library(ggplot2)
ggplot(AGG_Dark,aes(x=Flow.Rate, y=log10(smooth.vel), fill=Chemical.Cue))+
  geom_boxplot(notch=F, notchwidth=0.3,outlier.shape=1,outlier.size=2, coef=1.5)+
  stat_summary(geom = "errorbar", fun.min = mean, fun = mean, fun.max = mean, width = .75, linetype = "dashed", position = position_dodge())+
  theme_classic()+
  ...
  scale_fill_manual(values=c("white", "gray", "slategray"),name = "Chemical Cue",
...
#########    our data using nlme package     ######### ## block trial, not track

mode1 <- lme(pitch.perfect~Chem*Light*Flow.rate, random= ~1|D_V_T, data=Feeding)

mode2 <- lme(pitch.perfect~Light*Flow.rate, random= ~1|D_V_T, data=Feeding)
...
model1 <- lmer(vel.flow ~ Flow.rate*Chem*Light + (1|D_V_T) + (1|Flow.rate:D_V_T) + (1|Chem:D_V_T) +  (1|Light:D_V_T), data=Feeding)  ###

model2 <- lmer(vel.flow ~ Flow.rate*Light + (1|D_V_T) + (1|Flow.rate:D_V_T) +  (1|Light:D_V_T), data=Feeding)
...
plot(effect("vel.flow*Flow.rate",se=TRUE, confidence.level=.95, mode2))


###### interaction plot with Lattice extra #########
library(latticeExtra)
xyplot(Feeding$vel.flow~Feeding$turn.angle|Feeding$Light*Feeding$Flow.rate,  main="Scatterplots by Light and Flow Rate", 
...
interaction.plot(Feeding$vel.flow, ## x axis
                 Feeding$Flow.rate, ## factor for lines
                 Feeding$turn.angle,  ## y axis
                 fun = mean,
...
Feeding[is.nan(Feeding)] <- 0
range(Feeding$log.vel.flow)
##########################################################
...
mod1 <- lm(vel.flow~Flow.rate*Chlorophyll, data = Feeding)
summary(mode2)
...
Horizontal Headings for each level of Flow
...
Horizontal Headings for each level of Chlorophyll
...
Using subset data from notebook 11
Plotting histograms for each condition level, starting with turn angles, Horizontal and Vertical Heading, and Velocity next...
```{r}
par(mar= c(4,4,2,1))
par(mfrow=c(1,3))


hist_info <- hist(DHN$turn.anglexysmooth, plot = FALSE)         # Store output of hist function
hist_info$density <- hist_info$counts /    # Compute density values
  sum(hist_info$counts) * 100
plot(hist_info, freq = FALSE, xlab = "Turn Angle (degrees)", ylab = "Percentage (%)", main = "Dark, High Flow (5.9 cm s-1), No Chemicals", xlim=c(0,100), ylim=c(0,75), xaxp = c(0, 180, 18))              # Plot histogram with percentages

hist_info <- hist(DHG$turn.anglexysmooth, plot = FALSE)         # Store output of hist function
hist_info$density <- hist_info$counts /    # Compute density values
  sum(hist_info$counts) * 100
plot(hist_info, freq = FALSE, xlab = "Turn Angle (degrees)", ylab = "Percentage (%)", main = "Dark, High Flow (5.9 cm s-1), Guano Only", xlim=c(0,100), ylim=c(0,75), xaxp = c(0, 180, 18))              # Plot histogram with percentages

hist_info <- hist(DHC$turn.anglexysmooth, plot = FALSE)         # Store output of hist function
hist_info$density <- hist_info$counts /    # Compute density values
  sum(hist_info$counts) * 100
plot(hist_info, freq = FALSE, xlab = "Turn Angle (degrees)", ylab = "Percentage (%)", main = "Dark, High Flow (5.9 cm s-1), Guano and Chlorophyll", xlim=c(0,100), ylim=c(0,75), xaxp = c(0, 180, 18))              # Plot histogram with percentages
...
range(AGG_Dark$pitch.perfect)

m_null <- mle2(pitch.perfect ~ dvm(exp(log_mu), exp(log_kappa)),
           parameters = list(log_mu ~ 1),
           data = AGG_Dark,
           start = list(log_mu = 0, log_kappa = 0))

m3_FC <- update(m_null, parameters = list(log_mu ~ Flow.rate+Chem))
