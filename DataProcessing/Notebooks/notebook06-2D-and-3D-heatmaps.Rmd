...
s = smooth.2d(CC.TotalData$Z, x=cbind(CC.TotalData$Y, CC.TotalData$X), theta=0.3)
image.plot(s, axes=TRUE, xlab = "Y", ylab = "X", legend.lab = "Z", legend.width = .5, legend.line = 3)


##changing Flow Rate into a numeric vector to plot it as a smooth heat map
CC.TotalData$Flow.rate<-as.numeric(CC.TotalData$Flow.rate)
## need to find how to do a stacked heat map like what Kuvvat showed in Python (https://stackoverflow.com/questions/57502763/how-to-plot-horizontal-stack-of-heatmaps-or-a-stack-of-grid)

s = smooth.2d(CC.TotalData$Flow.rate, x=cbind(CC.TotalData$Y, CC.TotalData$X), theta=0.3)
image.plot(s, axes=TRUE, xlab = "Y", ylab = "X", legend.lab = "Flow Rate (m/s)", legend.width = .5, legend.mar=8, legend.line = 4)

################################################################################################################################

################################################################################################################################

##changing Chlorophyll into a numeric vector to plot it as a smooth heat map
CC.TotalData$Chlorophyll<-as.numeric(CC.TotalData$Chlorophyll)
## need to find how to do a stacked heat map like what Kuvvat showed in Python (https://stackoverflow.com/questions/57502763/how-to-plot-horizontal-stack-of-heatmaps-or-a-stack-of-grid)

s = smooth.2d(CC.TotalData$Chlorophyll, x=cbind(CC.TotalData$X, CC.TotalData$Y), theta=0.3)
image.plot(s, axes=TRUE, xlab = "X", ylab = "Y", legend.lab = "Chlorophyll (mg/L)", legend.width = .5, legend.mar=8, legend.line = 4)

################################################################################################################################

################################################################################################################################

##changing Guano into a numeric vector to plot it as a smooth heat map
CC.TotalData$Guano<-as.numeric(CC.TotalData$Guano)  ##Nans produced 
...
```{r}
save.image("~/Post-doc/Data/Total Merged Data File (April 18 2022).RData")
