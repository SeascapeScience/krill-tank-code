...
data <- TotalData
str(data)
levels(data$Flow.rate) <- c("No Flow","Low Flow (0.6)","Low Flow (1.5)", "Medium Flow (3)", "High Flow (3.4)", "High Flow (5.9)","Extreme Flow (8.9)")
...
data$Flow.Direction <- ordered(data$Flow.Direction, levels = c("No Flow", "Upwelling", "Downwelling", "Horizontal"))
...
a <- ggplot(data, aes(x = Flow.rate, y = log(v), fill= Chlorophyll))+
  ...
  labs(size = "", x = "Flow Rate (mm s-1)", y = "Ground Velocity (logged, cm s-1)", title = "") +
  scale_fill_manual(values=c("white", "gray88", "slategray"),name = "Chemical Cue",
                    labels=c("No Chlorophyll", "Low Chlorophyll", "High Chlorophyll"))+
  facet_grid(Flow.Direction ~ Flow.rate, labeller = label_both, scales = "free_x", space = "free")
a

b <- ggplot(data,aes(x = Flow.rate, y = h.heading.pi, fill= Chlorophyll))+
  geom_boxplot(notch=F, notchwidth=0.3,outlier.shape=1,outlier.size=2, coef=1.5)+
  theme(legend.key.size = unit(2, "cm"))+
  theme(text = element_text(size = 16))+
  ##stat_summary(fun=mean, geom="line", lty=2, lwd=1, col = "black", aes(group=1)) +
  ...
  theme(axis.line = element_line(size = 0.5, linetype = "solid",
                                   colour = "black"))+
  theme(legend.key = element_rect(fill = "white"))+
  labs(size = "", x = "Chemical Cue", y = "Horizontal Heading (degrees)", title = "") +
  scale_fill_manual(values=c("white", "gray", "gray35"),name = "Chemical Cue",
                    labels=c("No Chlorophyll", "Low Chlorophyll", "High Chlorophyll"))+
  facet_grid(Flow.Direction ~ Light, labeller = label_both, scales = "free_x", space = "free")
b
...
circ.turn <- circular(AGG_Dark$turn.anglexysmooth, type = c("angles"),
          units = c("degrees"),
...
aov.circular(circ.turn, AGG_Dark$Flow.rate) ## working but unsure
