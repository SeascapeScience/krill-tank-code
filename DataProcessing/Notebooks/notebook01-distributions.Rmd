...
load("C:\\Users\\nicoleh3\\Documents\\Post-doc\\Data\\Total Data Merged File (Sep 7 2021).Rda")
...
tab <- matrix(data = NA, nrow = 113, ncol =5, byrow = T)
...
s.v <- rbind(s.v, s)
  ...
  hist(log10(TotalData$v[TotalData$D_V_T==ind[i]]),
       breaks = 50,
     xlab = "Velocity (Log^10 mm/s)",
     main = d.p)  ## change to d$p.value to print the p-value as the title instead
}
...
```{r}
##agregating TotalData so we can merge it with tab data

AGG_TD <- aggregate(TotalData, by = list(TotalData$D_V_T), FUN = mean)
head(AGG_TD)
AGG_TD <- AGG_TD[ -c(2:3, 8:10, 13:14) ]
colnames(AGG_TD) <- c("Ind", "X", "Y", "Z", "Track", "Flow.Rate", "Chlorophyll", "dx", "dy", "dz", "d", "vx", "vy", "vz", "v", "heading", "pitch", "turn.anglexy", "turn.angleyz", "turn.angle")
head(AGG_TD)
tail(AGG_TD)
