...
range(AGG_Dark$heading.pi)

AGG_Dark %>%
  filter(Chem %in% c("Ambient", "Guano", "Guano+CHL")) %>%
  ggplot(aes(x = heading.pi, fill = Chem))+
  ...
  theme(axis.line = element_line(size = 0.5, linetype = "solid",
                                   colour = "black"))+
  theme(legend.key = element_rect(fill = "white"))+
  labs(x = "Heading (degrees)", y = "Frequency of Heading", title = "")+
...
m_null <- mle2(heading.pi ~ dvm(exp(log_mu), exp(log_kappa)),
...
circ.turn <- circular(AGG_Dark$heading.pi, type = c("angles"),
          units = c("degrees"),
...
circDM <- circular(AGG_DM$turn.anglexysmooth, type = c("angles"),
          units = c("degrees"),
          template = c("geographics"))
...
data1 <- list(dahi = circular(AGG_DH$heading.pi, units="degrees", template="geographics"), dame = circular(AGG_DM$heading.pi, units="degrees", template="geographics"))
...
AGG_Dark$head.bins <- cut(AGG_Dark$heading.pi, breaks=c(-90,-75,-60,-45,-30,-15,0,15,30,45,60,75,90), labels=c("-90--75","-75--60","-60--45", "-45--30", "-30--15", "-15-0","0-15","15-30","30-45", "45-60", "60-75", "75-90"))
str(AGG_Dark)
g.test(AGG_Dark$Chem, AGG_Dark$Flow.rate)
