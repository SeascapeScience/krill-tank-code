...
# plot
ggplot(data = dat, aes(xsmooth, ysmooth, colour = Light)) +
  geom_spoke(data = dat, aes(x = abs(xsmooth), y = abs(ysmooth), angle = turn.anglexysmooth, radius = n/2), arrow=arrow(length = unit(0.2,"cm")), size = 1) +
  coord_equal()+
  theme_classic()+
  scale_color_manual(values = c("gray", "black"))+
  labs(size = "", x = "Mean distance in X axis", y = "Mean distance in Y axis", title = "Turn Angles as mean vector") +
  facet_grid(facets = Chem ~ Flow, labeller = label_both)

###########################################################################################
colour=c("grey", "black")[factor(dat$Light)]
```
