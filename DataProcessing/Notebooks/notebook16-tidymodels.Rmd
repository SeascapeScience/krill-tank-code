...
for (j in colnames(parameters)){
    ...
    print(colnames(df))
      ...
      for (i in 1:10){  
        ...
        c.e <- rf.skill(df = df, trees = 2000, col1 = colnames(df)[5]) ## try neural net vs random forest and other models
        c.v <- rbind(c.e, c.v)  
        ...
        output.test <- model.test(df = df, trees = 2000, col1 = colnames(df)[5], prop = 0.8, strata = NULL, do.plot = FALSE)
