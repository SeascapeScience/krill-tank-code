...
for (i in 1:10){  
  ...
  c.v <- rbind(c.e, c.v)  
  c.e.t <- rf.skill.test(df = df, trees = 2000, col1 = "v.head dip test", prop = 0.8, strata = NULL, do.plot = TRUE) ## training data
  c.v_train <- rbind(c.e.t, c.v_train)
  rsq1 <- rf.skill(df = df, trees = 2000, col1 = "v.head dip test")
