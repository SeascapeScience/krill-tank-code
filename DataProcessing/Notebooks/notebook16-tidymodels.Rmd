...
rf_split <- initial_split(df %>% select(flow, chl, guano, light, ave.v), ## splits cases base on initial results
                            strata = flow)  ## with strata = NULL splits 11/31, rf_test has all guano absent, 50/50 light split, uneven flow split (0, 3, 3, 5.9, 5.9, 5.9, 8.9 x5), and random chl values.
...
for (i in 1:length(10)){
  ...
  jpeg(filename= paste('~/Documents/Bigelow/Figures/tidymodels Outputs/', rf_wf[i],'.jpeg', sep = ''), width = 960, height = 780)
  plot(df$ave.v, rf_pred$.pred, main = paste("corr = ", cor$estimate))  ## observed vs predicted
  dev.off()
