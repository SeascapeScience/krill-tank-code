...
for (i in 1:10){  
  ...
  c.e <- rf.skill(df = df, trees = 2000, "dip.test") ## try neural net vs random forest and other models
  c.v <- rbind(c.e, c.v)  
  c.e.t <- rf.skill.test(df = df, trees = 2000, "dip.test", prop = 0.5, strata = NULL, do.plot = TRUE) ## training data
  c.v_train <- rbind(c.e.t, c.v_train)
  
  ggsave(filename=paste('~/Bigelow/Figures/tidymodels Output/rf_train', i, '_2.jpeg', sep = ''), plot = p, width = 24 , height = 12)
