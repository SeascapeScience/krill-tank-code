##### 3D plotting ######
rm(list=ls(all=TRUE))
krill <- read.csv("./Data/MASTER XYZ FACTOR FILE with d values (Jan 4 2021).csv")
head(krill)
str(krill)
...
# This code replaces the for-loop computing d (distance moved)

x1 <- krill$x[1:(nrow(krill)-1)]
x2 <- krill$x[2:nrow(krill)]
krill$dx[1:(nrow(krill)-1)] <- x2-x1

y1 <- krill$y[1:(nrow(krill)-1)]
y2 <- krill$y[2:nrow(krill)]
krill$dy[1:(nrow(krill)-1)] <- y2-y1

z1 <- krill$z[1:(nrow(krill)-1)]
z2 <- krill$z[2:nrow(krill)]
krill$dz[1:(nrow(krill)-1)] <- z2-z1

krill$d <- sqrt(krill$dx^2 + krill$dy^2 + krill$dz^2)
...
krill$vx <- krill$dx/(1/30)  ## velocity on the x axis
krill$vy <- krill$dy/(1/30)  ## velocity on the y axis
krill$vz <- krill$dz/(1/30)  ## velocity on the z axis
krill$v <- krill$d/(1/30) ## total velocity

# Should just be dx/unit time???
...
krill$heading <- atan(krill$dx/krill$dy)
krill$pitch <- atan(krill$dz/(sqrt(krill$dx^2 + krill$dy^2)))


hist(krill$pitch/(2*pi)*360)
hist(krill$heading/(2*pi)*360)

hist(krill$dz[abs(krill$dz)<.01],breaks = 100)
hist(krill$dy[abs(krill$dy)<.01],breaks = 100)
hist(krill$dx[abs(krill$dx)<.01],breaks = 100)
